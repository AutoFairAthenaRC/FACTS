...
def incorrectRecoursesIfThen_NN(
    ifclause: Predicate,
...
def incorrectRecoursesSingle(
    sd: Predicate, h: Predicate, s: Predicate, X_aff: DataFrame, model: ModelAPI
) -> int:
    ...
    X_aff_covered = X_aff[
        X_aff.apply(lambda x: h.satisfies(x) and sd.satisfies(x), axis=1)
    ].copy()
    if X_aff_covered.shape[0] == 0:
        return 0
...
def incorrectRecourses(
    R: TwoLevelRecourseSet, X_aff: DataFrame, model: ModelAPI
) -> int:
    new_rows = []
...
def incorrectRecoursesSubmodular(
    R: TwoLevelRecourseSet, X_aff: DataFrame, model: ModelAPI
) -> int:
...
def featureCost(R: TwoLevelRecourseSet):
    return sum(
        ...
        for h, s in zip(r.hypotheses, r.suggestions)
    )


def featureChange(R: TwoLevelRecourseSet):
    return sum(
        featureChangePred(h, s)
        for r in R.rules.values()
        for h, s in zip(r.hypotheses, r.suggestions)
    )
...
def if_group_cost_mean_with_correctness(
    ...
    return -np.mean(
        [
            cor / featureChangePred(ifclause, thenclause, params=params)
...
def if_group_cost_mean_correctness_weighted(
    ...
    feature_changes = np.array(
        [
            ...
            for thenclause, _ in thenclauses
        ]
    )
...
def if_group_average_recourse_cost_cinf(
    ...
    ret = (
        np.dot(corr_diffs, costs)
        + (correctness_caps[ifclause] - cumulative_corrs[-1])
        * c_infty_coeff
        * costs.max()
    )
