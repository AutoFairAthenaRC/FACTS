...
from ..predicate import Predicate, featureCostPred, featureChangePred, recIsValid, drop_two_above
from ..parameters import feature_change_builder
...
def test_featureCostPred() -> None:
    ifclause = Predicate.from_dict({"a": 1, "b": 2, "c": 3})
    thenclause = Predicate.from_dict({"a": 1, "b": 4, "c": 7})
    params = ParameterProxy(featureCosts={"a": 1000, "b": 200, "c": 3})

    assert featureCostPred(ifclause, thenclause, params=params) == 203
